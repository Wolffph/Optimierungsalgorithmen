...
for i in range(0, len(xValues), 4):
    ...
    plt.plot([xValues[i + 2], xValues[i + 3]], [yValues[i + 2], yValues[i + 3]], 'b', linewidth=rect_linewidth)  # upper edges

    plt.plot([xValues[i], xValues[i]], [yValues[i], yValues[i + 2]], 'b', linewidth=rect_linewidth)  # left edges

    plt.plot([xValues[i + 1], xValues[i + 1]], [yValues[i], yValues[i + 2]], 'b', linewidth=rect_linewidth)  # right edges

# Plot the bounding boxes
for i in range(0, len(boxXValues), 4):
    plt.plot([boxXValues[i], boxXValues[i + 1]], [boxYValues[i], boxYValues[i]], 'orange', linewidth=box_linewidth)  # lower edges
    plt.plot([boxXValues[i + 2], boxXValues[i + 3]], [boxYValues[i + 2], boxYValues[i + 3]], 'orange', linewidth=box_linewidth)  # upper edges
    plt.plot([boxXValues[i], boxXValues[i]], [boxYValues[i], boxYValues[i + 2]], 'orange', linewidth=box_linewidth)  # left edges
    plt.plot([boxXValues[i + 1], boxXValues[i + 1]], [boxYValues[i], boxYValues[i + 2]], 'orange', linewidth=box_linewidth)  # right edges
